package edu.iastate.cs228.hw2;

/**
 * 
 * @author Bryan Pope
 *
 */

public class Point implements Comparable<Point> {
	private int x;
	private int y;

	public static boolean xORy; // compare x coordinates if xORy == true and y coordinates otherwise
								// To set its value, use Point.xORy = true or false.

	/**
	 * This is the default constructor setting both X and Y to 0
	 */
	public Point() { // default constructor
		this.x = 0;
		this.y = 0;
	}

	/**
	 * A constructor of point using x and y as input
	 * 
	 * @param x
	 * @param y
	 */
	public Point(int x, int y) {
		this.x = x;
		this.y = y;
	}

	/**
	 * @param p the point object to be copied This constructor creates a copy of the
	 *          point
	 */
	public Point(Point p) { // copy constructor
		this.x = p.getX();
		this.y = p.getY();
	}

	/**
	 * @return The value of x
	 */
	public int getX() {
		return x;
	}

	/**
	 * @return the value of y
	 */
	public int getY() {
		return y;
	}

	/**
	 * Set the value of the static instance variable xORy.
	 * 
	 * @param xORy
	 */
	public static void setXorY(boolean xORy) {
		if (xORy) {
			Point.xORy = true;
		} 
		else {
			Point.xORy = false;
		}
	}

	@Override
	public boolean equals(Object obj) {
		if (obj == null || obj.getClass() != this.getClass()) {
			return false;
		}

		Point other = (Point) obj;
		return x == other.x && y == other.y;
	}

	/**
	 * Compare this point with a second point q depending on the value of the static
	 * variable xORy
	 * 
	 * @param q
	 * @return -1 if (xORy == true && (this.x < q.x || (this.x == q.x && this.y <
	 *         q.y))) || (xORy == false && (this.y < q.y || (this.y == q.y && this.x
	 *         < q.x))) 0 if this.x == q.x && this.y == q.y) 1 otherwise
	 */
	public int compareTo(Point q) {
		// Returns -1 if xORy searching for X, and either x is less than the compared
		// point is x, otherwise it checks y
		if (xORy == true && (this.x < q.x || (this.x == q.x && this.y < q.y))) {

			return -1;
		}
		// Returns -1 if XorY is searching for 7, and either y is less than the compared
		// point otherwise it checks y
		else if (xORy == false && (this.y < q.y || (this.y == q.y && this.x < q.x))) {
			return -1;
		}
		// returns 0 if they're equal
		else if (this.x == q.x && this.y == q.y) {
			return 0;
		} else {
			return 1;
		}
	}

	/**
	 * Outputs a point in the standard form (x, y).
	 */
	@Override
	public String toString() {
		return "(" + this.x + ", " + this.y + ")";
	}
}
